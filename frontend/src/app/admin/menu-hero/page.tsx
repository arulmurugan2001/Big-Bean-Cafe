...
export default function MenuHeroList() {
  ...
  const deleteItem = async (id: number) => {
    if (!confirm('Delete this menu hero banner?')) return
    try {
      const res = await apiRequest(`/menu-hero/${id}`, {
        method: 'DELETE',})
      if (res.ok) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Menu Page Hero Banner</h1>
          <p className="text-gray-600 mt-1">Manage the hero banner displayed on the Menu page</p>
        </div>
        <Link
          href="/admin/menu-hero/add"
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500">
          ...
          <p>No menu hero banners yet.</p>
          <Link href="/admin/menu-hero/add" className="mt-4 inline-block text-[#C9943A] font-semibold hover:underline">Add your first one</Link>
        </div>
      ) : (
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Link href={`/admin/menu-hero/${item.id}/edit`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
