...
export default function EditMenuHero() {
    ...
    const load = async () => {
      try {
        ...
        if (data.success && data.data) {
          const d = data.data
          setForm({
            eyebrow: d.eyebrow || 'BIG BEAN CAFÉ MENU',
            title: d.title || '',
            highlight_text: d.highlight_text || '',
            subtitle: d.subtitle || '',
            button_primary_text: d.button_primary_text || 'Order Online',
            button_primary_url: d.button_primary_url || 'https://bigbeancafe.store',
            button_secondary_text: d.button_secondary_text || 'Explore Menu',
            button_secondary_url: d.button_secondary_url || '#menu-products',
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Menu Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Update the Menu page hero banner</p>
        </div>
      </div>
