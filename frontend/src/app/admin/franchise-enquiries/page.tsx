...
import { Search, Eye, X, MapPin, DollarSign, MessageCircle } from 'lucide-react'
...
interface Enquiry {
  ...
  city: string; state: string; investment_range: string | null
  preferred_location: string | null; business_experience: string | null
...
export default function AdminFranchiseEnquiries() {
  ...
  const fetchEnquiries = async () => {
    try {
      ...
      const res = await apiRequest(`/franchise-enquiries?${params}`, {})
  ...
  const updateStatus = async (id: number, status: string) => {
    setUpdatingId(id)
    try {
      await apiRequest(`/franchise-enquiries/${id}/status`, {
        method: 'PUT',
  ...
  const handleDelete = async (id: number) => {
    ...
    await apiRequest(`/franchise-enquiries/${id}`, { method: 'DELETE',})
    fetchEnquiries()
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-3xl font-bold text-gray-900">Franchise Enquiries</h1><p className="text-gray-500 text-sm mt-1">{enquiries.length} enquir{enquiries.length !== 1 ? 'ies' : 'y'}</p></div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" placeholder="Search name, email, phone, city..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      {loading ? <div className="text-center py-16 text-gray-400">Loading...</div> : enquiries.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  <th className="text-left px-6 py-4 font-semibold text-gray-600">Enquirer</th>
                  <th className="text-left px-6 py-4 font-semibold text-gray-600">Location</th>
                  <th className="text-left px-6 py-4 font-semibold text-gray-600">Investment</th>
                  <th className="text-left px-6 py-4 font-semibold text-gray-600">Preferred Area</th>
                ...
                {enquiries.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{e.email}</p>
                      <p className="text-xs text-gray-400">{e.phone}</p>
                    </td>
                    <td className="px-6 py-4 text-gray-700">{e.city}, {e.state}</td>
                    <td className="px-6 py-4 text-gray-700 text-xs">{e.investment_range || '—'}</td>
                    <td className="px-6 py-4 text-gray-500 text-xs max-w-[160px]"><p className="truncate">{e.preferred_location || '—'}</p></td>
                    <td className="px-6 py-4">
                      <Can module="franchise_enquiries" action="edit">
                        <div className="relative group">
                          ...
                          <div className="absolute left-0 top-7 z-10 hidden group-hover:flex flex-col bg-white border border-gray-100 rounded-xl shadow-lg overflow-hidden min-w-[130px]">
                            {['new','contacted','qualified','rejected','converted'].map(s => (
                      ...
                      <Can module="franchise_enquiries" action="edit" fallback={(
                    ...
                    <td className="px-6 py-4 text-xs text-gray-500">{formatDate(e.created_at)}</td>
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-1 items-center">
                        {e.phone && (
                          <a href={`https://wa.me/${e.phone.replace(/\D/g,'')}?text=${encodeURIComponent(`Hi ${e.full_name}, thank you for your interest in a Big Bean Café franchise! Our team will be in touch shortly. ☕`)}`}
                        ...
                        <Link href={`/admin/franchise-enquiries/${e.id}`}
                        ...
                        <Can module="franchise_enquiries" action="delete">
                          <button onClick={() => handleDelete(e.id)} className="p-2 hover:bg-red-50 rounded-lg text-red-500" title="Delete"><X className="w-4 h-4" /></button>
                        </Can>
                      </div>
                    </td>
                  </tr>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={ex => ex.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">{selected.city}, {selected.state}</p>
              </div>
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">City</p><p className="text-gray-800">{selected.city}</p></div>
                <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">State</p><p className="text-gray-800">{selected.state}</p></div>
                <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Investment Range</p><p className="text-gray-800">{selected.investment_range || '—'}</p></div>
              ...
              {selected.preferred_location && <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Preferred Location</p><p className="text-sm text-gray-700 whitespace-pre-wrap">{selected.preferred_location}</p></div>}
              {selected.business_experience && <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Business Experience</p><p className="text-sm text-gray-700 whitespace-pre-wrap">{selected.business_experience}</p></div>}
              {selected.message && <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Message</p><p className="text-sm text-gray-700 whitespace-pre-wrap">{selected.message}</p></div>}
