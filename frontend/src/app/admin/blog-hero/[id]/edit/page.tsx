...
export default function EditBlogHero() {
  ...
  useEffect(() => {
    apiRequest(`/blog-hero/${id}`)
      .then(r => r.json())
      .then(data => {
        if (data.success && data.data) {
          const d = data.data
          setForm({
            eyebrow: d.eyebrow || '', title: d.title || '', highlight_text: d.highlight_text || '',
            subtitle: d.subtitle || '',
            button_primary_text: d.button_primary_text || '', button_primary_url: d.button_primary_url || '',
            button_secondary_text: d.button_secondary_text || '', button_secondary_url: d.button_secondary_url || '',
            stat_1_value: d.stat_1_value || '', stat_1_label: d.stat_1_label || '',
            stat_2_value: d.stat_2_value || '', stat_2_label: d.stat_2_label || '',
            stat_3_value: d.stat_3_value || '', stat_3_label: d.stat_3_label || '',
            status: d.status || 'active', sort_order: String(d.sort_order ?? 0)
          })
          if (d.image) {
            const url = d.image.startsWith('http') ? d.image : `${API_BASE_URL}/${d.image.replace(/^\/+/, '')}`
            setImagePreview(url)
          }
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiRequest(`/blog-hero/${id}`, {
        method: 'PUT',
        body: fd
      })
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Blog Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Update hero banner content and image</p>
        </div>
      </div>
