...
export default function AddFranchiseHero() {
  const router = useRouter()
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [imageFile, setImageFile] = useState<File | null>(null)
  const [form, setForm] = useState({
    eyebrow: 'FRANCHISE WITH BIG BEAN CAFÉ', title: 'Build a Coffee Business',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiRequest('/franchise-hero', { method: 'POST', body: fd })
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Add Franchise Hero Banner</h1>
      </div>
