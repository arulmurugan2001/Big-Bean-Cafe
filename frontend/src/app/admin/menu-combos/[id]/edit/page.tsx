...
export default function EditMenuCombo() {
  ...
  const [form, setForm] = useState({
    ...
    price: '', mrp: '', badge_text: 'Chef Recommended',
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiRequest(`/menu-combos/${id}`, {})
        const data = await res.json()
        if (data.success && data.data) {
          const d = data.data
          setForm({
            title: d.title || '', subtitle: d.subtitle || '',
            description: d.description || '', items_text: d.items_text || '',
            price: d.price != null ? String(d.price) : '',
            mrp: d.mrp != null ? String(d.mrp) : '',
            badge_text: d.badge_text || 'Chef Recommended',
            button_text: d.button_text || 'Order Combo',
            button_url: d.button_url || 'https://bigbeancafe.store',
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Menu Combo</h1>
          <p className="text-gray-500 text-sm mt-1">Update combo deal details</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <input className={inputClass} value={form.items_text} onChange={e => set('items_text', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>Badge Text</label>
              <input className={inputClass} value={form.badge_text} onChange={e => set('badge_text', e.target.value)} />
            </div>
            <div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input type="number" step="0.01" className={inputClass} value={form.mrp} onChange={e => set('mrp', e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input className={inputClass} value={form.button_text} onChange={e => set('button_text', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>Button URL</label>
              <input className={inputClass} value={form.button_url} onChange={e => set('button_url', e.target.value)} />
            </div>
            <div>
