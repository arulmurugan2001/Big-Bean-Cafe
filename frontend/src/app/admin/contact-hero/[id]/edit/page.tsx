...
export default function EditContactHero() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiRequest(`/contact-hero/${id}`, {})
        const data = await res.json()
        if (data.success && data.data) {
          const d = data.data
          setForm({
            eyebrow: d.eyebrow || 'CONTACT BIG BEAN CAFÉ',
            title: d.title || '',
            highlight_text: d.highlight_text || '',
            subtitle: d.subtitle || '',
            button_primary_text: d.button_primary_text || 'Send Enquiry',
            button_primary_url: d.button_primary_url || '#contact-form',
            button_secondary_text: d.button_secondary_text || 'Find Outlet',
            button_secondary_url: d.button_secondary_url || '/outlets',
            stat_1_value: d.stat_1_value || '7+',
            stat_1_label: d.stat_1_label || 'Outlets',
            stat_2_value: d.stat_2_value || 'Fast',
            stat_2_label: d.stat_2_label || 'Support',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) router.push('/admin/contact-hero')
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/admin/contact-hero" className="p-2 hover:bg-gray-100 rounded-xl transition-colors">
          <ArrowLeft className="w-5 h-5 text-gray-600" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Contact Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Update the Contact page hero banner</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <Link href="/admin/contact-hero" className="px-8 py-3 rounded-xl border border-gray-200 font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
