...
export default function ContactHeroList() {
  const [items, setItems] = useState<ContactHero[]>([])
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Delete this contact hero banner?')) return
    await apiRequest(`/contact-hero/${id}`, {
      method: 'DELETE',})
    load()
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Contact Hero Banners</h1>
          <p className="text-gray-500 text-sm mt-1">Manage the Contact page hero banner</p>
        </div>
        <Link href="/admin/contact-hero/add"
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
          ...
          <Link href="/admin/contact-hero/add" className="mt-4 inline-block text-sm text-[#C9943A] font-semibold hover:underline">Add your first banner</Link>
        </div>
      ) : (
                  ...
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2 justify-end">
                        <Link href={`/admin/contact-hero/${item.id}/edit`}
