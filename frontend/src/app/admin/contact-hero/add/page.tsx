...
export default function AddContactHero() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiRequest('/contact-hero', {
        method: 'POST',
        body: fd
      })
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Add Contact Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Create a new Contact page hero banner</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <input className={inputClass} value={form.title} onChange={e => set('title', e.target.value)} placeholder="e.g. Let’s Brew" />
            </div>
            <div className="md:col-span-2">
              <label className={labelClass}>Highlight Text <span className="text-xs text-[#C9943A]">(shown in gold)</span></label>
              <input className={inputClass} value={form.highlight_text} onChange={e => set('highlight_text', e.target.value)} placeholder="e.g. Something Together" />
            </div>
            <div className="md:col-span-2">
