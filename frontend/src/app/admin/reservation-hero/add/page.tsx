...
export default function AddReservationHero() {
  ...
  const [form, setForm] = useState({
    ...
    title: 'Reserve Your',
    highlight_text: 'Perfect Café Moment',
    subtitle: 'Book your table at your nearby Big Bean Café outlet and enjoy fresh coffee, food and warm conversations.',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) router.push('/admin/reservation-hero')
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/admin/reservation-hero" className="p-2 hover:bg-gray-100 rounded-xl transition-colors">
          <ArrowLeft className="w-5 h-5 text-gray-600" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Add Reservation Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Create a new Reservations page hero banner</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <input className={inputClass} value={form.title} onChange={e => set('title', e.target.value)} placeholder="e.g. Reserve Your" />
            </div>
            <div className="md:col-span-2">
              <label className={labelClass}>Highlight Text <span className="text-xs text-[#C9943A]">(shown in gold)</span></label>
              <input className={inputClass} value={form.highlight_text} onChange={e => set('highlight_text', e.target.value)} placeholder="e.g. Perfect Café Moment" />
            </div>
            <div className="md:col-span-2">
        ...
        <div className="flex gap-4">
          ...
          <Link href="/admin/reservation-hero" className="px-8 py-3 rounded-xl border border-gray-200 font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
