...
export default function EditReservationHero() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiRequest(`/reservation-hero/${id}`, {})
        const data = await res.json()
        if (data.success && data.data) {
          const d = data.data
          setForm({
            eyebrow: d.eyebrow || 'TABLE RESERVATIONS',
            title: d.title || '',
            highlight_text: d.highlight_text || '',
            subtitle: d.subtitle || '',
            button_primary_text: d.button_primary_text || 'Reserve Table',
            button_primary_url: d.button_primary_url || '#reservation-form',
            button_secondary_text: d.button_secondary_text || 'View Outlets',
            button_secondary_url: d.button_secondary_url || '/outlets',
            stat_1_value: d.stat_1_value || '7+',
            stat_1_label: d.stat_1_label || 'Outlets',
            stat_2_value: d.stat_2_value || '30 Days',
            stat_2_label: d.stat_2_label || 'Advance Booking',
            stat_3_value: d.stat_3_value || 'Fast',
            stat_3_label: d.stat_3_label || 'Confirmation',
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Reservation Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Update the Reservations page hero banner</p>
        </div>
      </div>
