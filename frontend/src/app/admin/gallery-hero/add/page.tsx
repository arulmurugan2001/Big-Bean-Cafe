...
export default function AddGalleryHero() {
  ...
  const [form, setForm] = useState({
    eyebrow: 'BIG BEAN CAFÉ GALLERY',
    title: 'Moments Brewed',
    highlight_text: 'at Big Bean Café',
    subtitle: 'Explore our café stories, Instagram reels, outlet moments, events, coffee creations and community memories.',
    button_primary_text: 'Explore Gallery',
    button_primary_url: '#gallery-list',
    button_secondary_text: 'Follow Instagram',
    button_secondary_url: 'https://www.instagram.com/bigbeancafe.in/',
    stat_1_value: 'Reels',
    stat_1_label: 'Café Moments',
    stat_2_value: 'Events',
    stat_2_label: 'Workshops',
    stat_3_value: 'Outlets',
    stat_3_label: 'Good Vibes',
    status: 'active',
    sort_order: '0'
  })
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Add Gallery Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Create a new Gallery page hero banner</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input className={ic} value={form.eyebrow} onChange={e => set('eyebrow', e.target.value)} />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <input className={ic} value={form.title} onChange={e => set('title', e.target.value)} />
            </div>
            <div className="md:col-span-2">
              <label className={lc}>Highlight Text <span className="text-xs text-[#C9943A]">(shown in gold)</span></label>
              <input className={ic} value={form.highlight_text} onChange={e => set('highlight_text', e.target.value)} />
            </div>
            <div className="md:col-span-2">
              <label className={lc}>Subtitle</label>
              <textarea className={ic} rows={3} value={form.subtitle} onChange={e => set('subtitle', e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <button type="submit" disabled={saving}
            ...
            {saving ? 'Saving...' : 'Create Hero Banner'}
          </button>
          <Link href="/admin/gallery-hero" className="px-8 py-3 rounded-xl border border-gray-200 font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
