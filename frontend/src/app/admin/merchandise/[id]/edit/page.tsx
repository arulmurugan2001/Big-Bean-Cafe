...
export default function EditMerchandise() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiRequest(`/merchandise/${id}`, { method: 'PUT', body: fd })
  ...
  const currentImg = imagePreview || getImageUrl(existingImage)
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Merchandise</h1>
            ...
            <div className="card p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={form.slug} onChange={e => set('slug', e.target.value.toLowerCase().replace(/[^a-z0-9-]+/g, '').replace(/^-+|-+$/g, ''))} required
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-coffee-500" />
                <p className="mt-1 text-xs text-gray-400">Website URL: /merchandise/<span className="font-medium text-gray-600">{form.slug || 'slug'}</span></p>
              </div>
              <div>
              ...
              <div onClick={() => fileRef.current?.click()}
                ...
                {currentImg ? (
                  <div className="relative">
                    <img src={currentImg} alt="Preview" className="w-full h-48 object-cover rounded-lg" />
                    <button type="button"
