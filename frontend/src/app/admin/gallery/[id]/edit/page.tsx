...
export default function EditGalleryItem() {
  ...
  const [form, setForm] = useState({
    ...
    instagram_url: '', description: '', tags: '',
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiRequest(`/gallery-items/${id}`, {})
        const data = await res.json()
        if (data.success && data.data) {
          const d = data.data
          setForm({
            title: d.title || '', category: d.category || 'general',
            media_type: d.media_type || 'image',
            instagram_url: d.instagram_url || '', description: d.description || '',
            tags: d.tags || '', status: d.status || 'active',
            is_featured: !!d.is_featured, sort_order: String(d.sort_order ?? 0)
          })
          if (d.image) setImagePreview(d.image.startsWith('http') ? d.image : `${API_BASE}/${d.image.replace(/^\/+/, '')}`)
    ...
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Update this gallery item</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={form.is_featured} onChange={e => set('is_featured', e.target.checked)} className="w-4 h-4 accent-[#C9943A]" />
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          {form.media_type === 'image' && (
            <div>
              <label className={lc}>Image {!imagePreview && <span className="text-xs text-gray-400">(leave empty to keep existing)</span>}</label>
              ...
              <div>
                ...
                <label className="flex flex-col items-center gap-3 border-2 border-dashed border-gray-200 rounded-xl p-6 cursor-pointer hover:border-[#C9943A] transition-colors">
                  {videoFile ? (
                    ...
                    <p className="text-sm text-blue-600">Existing video (click to replace)</p>
                  ) : (
                    <>
                      <Upload className="w-8 h-8 text-gray-400" />
