...
export default function CreateEventPage() {
  ...
  const handleFileChange = (type: 'banner' | 'thumbnail', file: File | null) => {
    if (type === 'banner') {
      setBannerFile(file)
  ...
  const validate = (status: string = form.status): string => {
    ...
    if (status === 'active' && !form.dates.some(d => d.status === 'active')) return 'At least one active date is required for an active event'
  ...
  const handleSubmit = async (mode: 'draft' | 'active') => {
    ...
    try {
      ...
      const res = await apiRequest('/admin/events', { method: 'POST', body: fd })
  ...
  return (
    <div className="pb-28">
      ...
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-[#0F1F1A] sm:text-3xl">Add Event</h1>
          <p className="text-sm text-[#5F6F68]">Create a new Big Bean Cafe event</p>
        </div>
      </div>
