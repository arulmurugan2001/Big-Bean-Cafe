...
export default function AdminCorporateHero() {
  const [banners, setBanners] = useState<CorporateHero[]>([])
  ...
  const fetchBanners = async () => {
    try {
      const res = await apiRequest('/corporate-hero', {})
  ...
  const handleDelete = async (id: number) => {
    ...
    await apiRequest(`/corporate-hero/${id}`, { method: 'DELETE',})
    fetchBanners()
  }

  const toggleStatus = async (b: CorporateHero) => {
    const newStatus = b.status === 'active' ? 'inactive' : 'active'
    const fd = new FormData(); fd.append('title', b.title); fd.append('status', newStatus)
    await apiRequest(`/corporate-hero/${b.id}`, { method: 'PUT', body: fd })
    fetchBanners()
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-3xl font-bold text-gray-900">Corporate Hero</h1><p className="text-gray-500 text-sm mt-1">{banners.length} banner{banners.length !== 1 ? 's' : ''}</p></div>
        <Link href="/admin/corporate-hero/add" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-sm font-semibold shadow-sm" style={{ background: 'linear-gradient(to right,#C9943A,#8B4A2F)' }}>
      ...
      {loading ? <div className="text-center py-16 text-gray-400">Loading...</div> : banners.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <Link href="/admin/corporate-hero/add" className="mt-4 inline-block text-sm text-[#C9943A] font-semibold hover:underline">Add your first banner</Link>
        </div>
      ) : (
            ...
            <div key={b.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col sm:flex-row">
              ...
              <div className="flex-1 p-5 flex flex-col justify-between">
                ...
                <div className="flex items-center gap-2 mt-3 flex-wrap">
                  ...
                  <Link href={`/admin/corporate-hero/${b.id}/edit`} className="p-1.5 hover:bg-blue-50 rounded-lg text-blue-500"><Edit className="w-4 h-4" /></Link>
