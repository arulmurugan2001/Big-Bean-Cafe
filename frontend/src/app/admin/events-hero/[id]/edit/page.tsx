...
export default function EditEventsHero() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiRequest(`/admin/events-hero/${id}`)
        const data = await res.json()
        if (data.success && data.data) {
          const d = data.data
          setForm({
            eyebrow: d.eyebrow || 'Big Bean Café',
            title: d.title || '',
            subtitle: d.subtitle || '',
            button_text: d.button_text || 'Explore Events',
            button_link: d.button_link || '/events',
            overlay_opacity: String(d.overlay_opacity ?? 0.60),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) router.push('/admin/events-hero')
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/admin/events-hero" className="p-2 hover:bg-gray-100 rounded-xl transition-colors">
          <ArrowLeft className="w-5 h-5 text-gray-600" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Events Hero</h1>
          <p className="text-gray-500 text-sm mt-1">Update the Events page hero banner</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input className={inputClass} value={form.button_link} onChange={e => set('button_link', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>Overlay Opacity (0.1 - 1.0)</label>
              <input type="number" step="0.1" min="0.1" max="1" className={inputClass} value={form.overlay_opacity} onChange={e => set('overlay_opacity', e.target.value)} />
            </div>
            <div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <h2 className="text-lg font-bold text-gray-800 mb-5">Hero Image</h2>
        ...
        <div className="flex gap-4">
          ...
          <Link href="/admin/events-hero" className="px-8 py-3 rounded-xl border border-gray-200 font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
