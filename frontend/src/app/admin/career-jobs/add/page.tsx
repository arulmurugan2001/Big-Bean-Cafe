...
export default function AddCareerJob() {
  ...
  const handleOutletChange = (outletId: string) => {
    ...
    set('outlet_name', outlet ? outlet.name : '')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiRequest('/career-jobs', {
        method: 'POST',
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div><h1 className="text-3xl font-bold text-gray-900">Add Job Opening</h1><p className="text-gray-500 text-sm mt-1">Create a new career opportunity</p></div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
              ...
              <div><label className={lc}>Job Title <span className="text-red-500">*</span></label><input className={ic} value={form.title} onChange={e => set('title', e.target.value)} required placeholder="e.g. Senior Barista" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className={lc}>Department</label><input className={ic} value={form.department} onChange={e => set('department', e.target.value)} placeholder="e.g. Operations" /></div>
                <div><label className={lc}>Experience</label><input className={ic} value={form.experience} onChange={e => set('experience', e.target.value)} placeholder="e.g. 2-3 years" /></div>
                ...
                <div><label className={lc}>Salary Range</label><input className={ic} value={form.salary_range} onChange={e => set('salary_range', e.target.value)} placeholder="e.g. 3-5 LPA" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={lc}>Outlet</label>
                ...
                <div><label className={lc}>Location</label><input className={ic} value={form.location} onChange={e => set('location', e.target.value)} placeholder="e.g. Bangalore" /></div>
              </div>
              <div><label className={lc}>Short Description</label><textarea className={ic} rows={3} value={form.short_description} onChange={e => set('short_description', e.target.value)} placeholder="Brief overview of the role..." /></div>
            </div>

            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
              <h2 className="font-bold text-gray-800 text-base">Job Content</h2>
              <div>
                <label className={lc}>Responsibilities</label>
                <textarea className={`${ic} font-mono text-xs`} rows={6} value={form.responsibilities} onChange={e => set('responsibilities', e.target.value)} placeholder="Enter each responsibility on a new line:&#10;Prepare and serve coffee beverages&#10;Train junior staff&#10;Maintain cleanliness" />
                <p className={taHint}>Each line will be shown as a bullet point on the website</p>
              </div>
              <div>
                <label className={lc}>Requirements</label>
                <textarea className={`${ic} font-mono text-xs`} rows={6} value={form.requirements} onChange={e => set('requirements', e.target.value)} placeholder="Enter each requirement on a new line:&#10;2+ years barista experience&#10;Knowledge of espresso techniques&#10;Good communication skills" />
                <p className={taHint}>Each line will be shown as a bullet point on the website</p>
              </div>
              <div>
                <label className={lc}>Benefits</label>
                <textarea className={`${ic} font-mono text-xs`} rows={4} value={form.benefits} onChange={e => set('benefits', e.target.value)} placeholder="Free coffee and meals&#10;Training programs&#10;Career growth" />
                <p className={taHint}>Each line will be shown as a bullet point on the website</p>
              </div>
            </div>
          </div>
