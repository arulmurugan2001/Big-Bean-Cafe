...
export default function AddCategory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiRequest('/merchandise-categories', {
        method: 'POST',
        body: fd
      })
    ...
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-600">Create a new merchandise category</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-6 max-w-2xl">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving}
            className="btn-primary disabled:opacity-50">{saving ? 'Saving...' : 'Save Category'}</button>
