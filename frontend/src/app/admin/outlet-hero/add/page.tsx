...
export default function AddOutletHero() {
  ...
  const [form, setForm] = useState({
    eyebrow: 'BIG BEAN CAFÉ OUTLETS',
    title: '',
    highlight_text: '',
    subtitle: '',
    button_primary_text: 'Find Nearby Outlet',
    button_primary_url: '#outlet-list',
    button_secondary_text: 'Order Online',
    button_secondary_url: 'https://bigbeancafe.store',
    stat_1_value: '7+',
    stat_1_label: 'Outlets',
    stat_2_value: 'Bengaluru',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiRequest('/outlet-hero', {
        method: 'POST',
        body: fd
      })
      const data = await res.json()
      if (data.success) router.push('/admin/outlet-hero')
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/admin/outlet-hero" className="p-2 hover:bg-gray-100 rounded-xl transition-colors">
          <ArrowLeft className="w-5 h-5 text-gray-600" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Add Outlet Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Create a new Outlets page hero banner</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <input className={inputClass} value={form.title} onChange={e => set('title', e.target.value)} placeholder="Find Your Favourite" />
            </div>
            <div className="md:col-span-2">
              <label className={labelClass}>Highlight Text <span className="text-xs text-[#C9943A]">(shown in gold)</span></label>
              <input className={inputClass} value={form.highlight_text} onChange={e => set('highlight_text', e.target.value)} placeholder="Big Bean Café" />
            </div>
            <div className="md:col-span-2">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
            <div>
              ...
              <input className={inputClass} value={form.stat_1_value} onChange={e => set('stat_1_value', e.target.value)} placeholder="7+" />
            </div>
            <div>
              <label className={labelClass}>Stat 1 Label</label>
              <input className={inputClass} value={form.stat_1_label} onChange={e => set('stat_1_label', e.target.value)} placeholder="Outlets" />
            </div>
            <div>
              <label className={labelClass}>Stat 2 Value</label>
              <input className={inputClass} value={form.stat_2_value} onChange={e => set('stat_2_value', e.target.value)} placeholder="Bengaluru" />
            </div>
            <div>
              <label className={labelClass}>Stat 2 Label</label>
              <input className={inputClass} value={form.stat_2_label} onChange={e => set('stat_2_label', e.target.value)} placeholder="Locations" />
            </div>
            <div>
              <label className={labelClass}>Stat 3 Value</label>
              <input className={inputClass} value={form.stat_3_value} onChange={e => set('stat_3_value', e.target.value)} placeholder="Daily" />
            </div>
            <div>
              <label className={labelClass}>Stat 3 Label</label>
              <input className={inputClass} value={form.stat_3_label} onChange={e => set('stat_3_label', e.target.value)} placeholder="Fresh Brews" />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <Link href="/admin/outlet-hero" className="px-8 py-3 rounded-xl border border-gray-200 font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
