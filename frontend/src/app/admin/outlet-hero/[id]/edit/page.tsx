...
export default function EditOutletHero() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiRequest(`/outlet-hero/${id}`, {})
        const data = await res.json()
        if (data.success && data.data) {
          const d = data.data
          setForm({
            eyebrow: d.eyebrow || 'BIG BEAN CAFÉ OUTLETS',
            title: d.title || '',
            highlight_text: d.highlight_text || '',
            subtitle: d.subtitle || '',
            button_primary_text: d.button_primary_text || 'Find Nearby Outlet',
            button_primary_url: d.button_primary_url || '#outlet-list',
            ...
            stat_2_value: d.stat_2_value || 'Bengaluru',
            stat_2_label: d.stat_2_label || 'Locations',
            stat_3_value: d.stat_3_value || 'Daily',
            stat_3_label: d.stat_3_label || 'Fresh Brews',
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Outlet Hero Banner</h1>
          <p className="text-gray-500 text-sm mt-1">Update the Outlets page hero banner</p>
        </div>
      </div>
